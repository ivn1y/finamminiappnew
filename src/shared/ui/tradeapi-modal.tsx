...
interface TradeAPIModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const TradeAPIModal: React.FC<TradeAPIModalProps> = ({ isOpen, onClose }) => {
      ...
      <div className="w-full min-h-full flex flex-col px-[20px]">
        ...
        <div className="flex justify-center pt-[48px]">
          <div className="w-[150px] h-[150px] relative flex-shrink-0">
            <img
              src="/assets/products/Trade API.png"
              alt="Trade API"
        ...
        <div className="mt-[40px]">
          <div className="bg-[#2f2f37] rounded-[5px] p-[12px] w-full">
            <p className="text-[17px] font-medium text-white leading-[24px] tracking-[-0.17px] font-inter">
              Решение, созданное для трейдеров и финтех-компаний, которым нужен прямой доступ к торговым операциям, позициям и рыночным данным.
            </p>
          </div>
        </div>
        ...
        <button 
          onClick={() => window.open('https://telegra.ph/nn-10-28-7', '_blank')}
        ...
        <button 
          onClick={() => window.open('https://t.me/emil_ge', '_blank')}
